...
void ConfigureScenario::Init()
{
    ...
    Config::SetDefault ("ns3::WifiMacQueue::MaxPacketNumber", UintegerValue(1000000)); 						//Maximum number of packets supported by the Wifi MAC buffer
...
//    Config::SetDefault ("ns3::Ipv4L3Protocol::DefaultTtl", UintegerValue (4));
    ...
    m_scenarioObjectContainer->m_ipv4InterfaceContainer = m_scenarioObjectContainer->m_ipv4AddressHelper.Assign(m_scenarioObjectContainer->m_netDeviceContainer);
...
//        if (ipv4)
//        {
//            for (u_int8_t j = 0; j < (int) ipv4->GetNInterfaces (); j++)
//            {
...
void ConfigureScenario::LoadStaticRouting(Ipv4StaticRoutingHelper staticRouting) {
    ...
    NS_ASSERT_MSG(file, "File " << fileName << " not found.");
    {
        ...
        while (file >> nodeId >> destination >> nexthop >> interface >> metric) {
            ...
            Ipv4Address dstAddress = m_scenarioObjectContainer->m_nodeContainer.Get((int) destination)->GetObject<Ipv4 > ()->GetAddress(1, 0).GetLocal();
            Ipv4Address nextHopAddress = m_scenarioObjectContainer->m_nodeContainer.Get((int) nexthop)->GetObject<Ipv4 > ()->GetAddress(1, 0).GetLocal();

            NS_LOG_DEBUG("Node " << (int) nodeId << " IP address " <<
                    m_scenarioObjectContainer->m_nodeContainer.Get((int) nodeId)->GetObject<Ipv4 > ()->GetAddress(1, 0).GetLocal() 
    ...
    for (i = 0; i < (int) m_nodesVector.size(); i++) {
        ...
        if (m_nodesVector[i].transmitter) {
            for (iter = m_nodesVector[i].destinations.begin(); iter != m_nodesVector[i].destinations.end(); iter++, portOffset++) {
                ...
                m_scenarioObjectContainer->m_sourceAppContainer.Start(Seconds(500.0 + offset));
                m_scenarioObjectContainer->m_sourceAppContainer.Stop(Seconds(990.0));
                ...
                m_scenarioObjectContainer->m_sinkAppContainer.Stop(Seconds(990.0));
                // offset += 0.2;
...
std::string ConfigureScenario::ComposeTraceFileName(u_int32_t runCounter)
{
    ...
    ////Compose the YansWifiPhy-based trace file name
    // 1 - Routing mechanism
    fileName = "PHY_";
    // 2 - Transport layer
    ...
    if (m_deployment == FILE_BASED) {
        assert (m_outputTraceFileName.find(".conf"));
    ...
    // 5 - FER value for the error-prone links (i.e. middle, side, every link, etc.)
    sprintf(temp, "%1.2f_RUN_%03d", m_fer, runCounter + m_runOffset);
    fileName += "FER_" + string(temp) + ".tr";
